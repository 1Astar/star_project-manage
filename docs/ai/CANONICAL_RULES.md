...
1. **要求不够明确 → 先细问再做**（不必写 spec）  
2. **计划 / 项目需求直接写进 Star PM**（灵感 / 任务 / 演进，带板块；有则写之，无则跳过）  
3. **直接开做**，不要写 `docs/superpowers/specs/*` 或长篇计划文件  
4. 除非用户明确说「先设计 / 先方案 / 写设计文档」  
5. **验收 / 发版前测试**：对照用户**初始要求**与 PM 条目，再跑技术检查
