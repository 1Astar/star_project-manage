 ...
 * 1) 前一日有更新且未完成的 PM 需求
 * 2) 前一日变更会话里 ❌ 未完成项
 * 3) Studio 未完成任务（todo / in_progress / paused）
 * 排序：昨日变更优先 → 优先级 P0→P3 → 标题
 */
export async function getTomorrowAgenda(opts?: {
  /** 基准「今天」的上海日；默认现在。清单对应「明天要看」= 以今天为参照看昨天变更 */
  todayDay?: string;
  ...
  const yesterday = new Date(`${todayDay}T12:00:00+08:00`);
  yesterday.setDate(yesterday.getDate() - 1);
  const yesterdayDay = shanghaiDay(yesterday.toISOString());
  ...
  await Promise.all(
    pmProjects.map(async (pmProject) => {
      ...
      for (const req of bundle.requirements) {
        ...
        if (!inShanghaiDay(req.updated_at, yesterdayDay)) continue;
        const id = `pm:${req.id}`;
        ...
        items.push({
          ...
          source: "pm_req",
          reason: "yesterday_changed",
          reasonLabel: REASON_LABELS.yesterday_changed,
  ...
  for (const task of studioSnap.tasks) {
    ...
    // 若任务进度备注/标题暗示昨日，或项目 nextAction 昨日改过，仍标 open；
    // 没有 task.updatedAt 时：默认 open_task；若 completedAt 为空且 status in_progress 也纳入
    items.push({
      id,
      title: task.title,
      ...
      source: "studio_task",
      reason: "open_task",
      reasonLabel: REASON_LABELS.open_task,
  ...
  for (const i of items) projectMap.set(i.projectId, i.projectTitle);
  const projects = [...projectMap.entries()]
    .map(([id, title]) => ({ id, title }))
    .sort((a, b) => a.title.localeCompare(b.title, "zh-CN"));

  return { todayDay, yesterdayDay, items: filtered, projects };
}
