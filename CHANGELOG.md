...
- **早 09:00（上海）**：推「今日要做 / 推荐」（主线、阻塞、到期、Bug + 昨日未完 + Git 建议）
- **晚 18:30（上海）**：推「待你验收」板块汇总
- **空队列默认不推**；`finish` 仍不推（见 v1.13.1）
- Cron：Cloudflare/Vercel 用 UTC（`0 1` / `30 10`）；EdgeOne 用 `Asia/Shanghai`
...
- **收工不推微信**：`finish_change_session` 只进工作台待验 / 站内通知，**不再每次 PushPlus**
- **发版汇总推**：正式 `publish_release` 成功后推一条（本版板块 + 说明摘要 + Release/工作台链接）；`draft` 默认不推
...
- **发版对比与分方向迭代**：发版前对照上一 Tag/CHANGELOG；不同方向/修改/需求尽量分版；优先一方向一发；记录必有（skill §2.0 + CANONICAL）
- **收工续接与插队归队**：做完仍有未完成时，PM「下一步」+ 当面点名接下来接哪条；插队搁置须写清，做完必须接回原计划；不做/后期说清防双盲
- **Skill 入库**：补齐并更新 `version-management`、`defer-scope-record` 等与上述规则配套的正文

## v1.12.1 · 2026-08-11

- **PushPlus 工作台日报**：每日 cron 推待验收、Git 同步建议、今日待办、昨天未完成；Markdown 可点链接（根地址 `NEXT_PUBLIC_APP_URL`）
- **验收单条推送**：收工提醒改为绝对页面链接
- **AI 规则**：开新项目前先扫 GitHub 复用；写入 skill / CANONICAL
...
- **MCP `create_bug`**：建单时直接写入 `status`，避免 create 后再 update 时快照读不到报「Bug 不存在」

## v1.11.2 · 2026-08-04

- **站点入口**：去掉「进入站点」按钮；待验收 / 项目库点**项目名**超链接开站，同域名复用已开标签

## v1.11.1 · 2026-08-04

- **打开线上站点**：项目库卡片与待验收（列表/Side Peek）可一键进入项目正式域名
- **登录页**：去掉演示沙盘说明文案
- **页脚版权**：不再显示个人姓名，仅保留 Starry Studio
...
- **每日 sync-git 建议**：新 commit 模糊匹配需求，写入待确认建议（MCP list/confirm）
- **智能化验收 A+B+C**：收工默认进待验清单并提醒；用户免验或小修/bug 可自动通过
- **PushPlus 微信推送**：验收提醒与每日待验摘要；工作台浏览器通知
- **Migration**：`035_git_sync_suggestions.sql`（commit↔需求建议表）

## v1.10.61 · 2026-07-30

- **甘特只看未完成**：默认隐藏「完成 / 放弃」，可与模块、有日期筛选叠加
- **Bug 详情增强**：关联需求含需求池+看板可搜；可改创建/更新时间；多人补充评论；指派发站内通知，并可复制/打开链接手发
- **Migration**：`034_bug_comments.sql`（Bug 评论表）

## v1.10.60 · 2026-07-29
- **迭代双口径**：面板「按范围 / 按时间」——范围看 `iteration_id`，时间看 `completed_at∈起止`；`align_project_periods` 默认不再按提出日覆盖挂期（需 `assignIterations:true`）

## v1.10.59 · 2026-07-29
- **迭代状态文案**：结束日已过显示「延期」（原「已过期」）；补 V1.2 起止日；MCP `update_planning_iteration`

## v1.10.58 · 2026-07-29
- **列面板可调序**：添加列弹层内拖 ⠿ 调整列顺序，与表头同步

## v1.10.57 · 2026-07-29
- **表头 + 加列**：需求表最右侧固定 `+`，点开顶部即可添加列 / 勾选显示；拖 ⠿ 调列序（不再整格拖）

## v1.10.56 · 2026-07-29
- 需求树整理：只认 8 个主 epic；批量 upsert 避免超时；补关键词与假 epic 降级挂父

## v1.10.55 · 2026-07-29

- **需求树整理**：MCP `organize_star_pm_req_tree` 把散条挂到父 epic、同步板块到模块筛选项并挂 module_l1；`update_requirement` 支持 parentId/type/moduleL1Id

## v1.10.54 · 2026-07-29

- **指派可点选**：需求表「指派」列点击弹出人选；默认含「产品」（你）与「白昼」（我），可多选

## v1.10.53 · 2026-07-29

- **迭代计划**：新建默认开始日=今天（必填）；标签上显示起止日，空则标「未写开始日」

## v1.10.52 · 2026-07-29

- **需求状态**：生命周期增加「开发中」（想法→已规划→AI开发中→开发中→待验收→完成）
- **看板列筛选**：点列名只看该状态；点旁边空白（或再点列名）取消筛选
- **口头意见回填 Bug**：聊天里历史修改意见写入 Star PM Bug（10 条）；`create_task` 支持 `__CREATE_BUG__` 桥接

## v1.10.51 · 2026-07-29

- **板块进程**：灰字显示真正补充（跳过「自 Release 导入」备注）；点击打开 Side Peek 看全字段
- **灵感/演进同步需求池**：撞 `studio_idea_id` 唯一约束时按已存在跳过，不再报失败

## v1.10.50 · 2026-07-29

- **MCP create_bug / list_bugs**：聊天口头意见可直接写入项目 Bug 表

## v1.10.49 · 2026-07-29

- **待验收 Side Peek**：点标题弹出右侧抽屉看变更会话/需求细节；Peek 内可直接通过或打回

## v1.10.48 · 2026-07-29

- **当前主线算法**：按待验收/Bug/跟进/阻塞/近2日变更与演进等打分选主线；项目库「主线」状态改为钉主线加权

## v1.10.47 · 2026-07-29

- **今日清单布局**：验收∥跟进；未关 Bug∥明日待办（两行并排）

## v1.10.46 · 2026-07-29

- **工作台布局**：日历 → 项目库（默认收起）→ 今日清单 + 明日待办；去掉「现场」

## v1.10.45 · 2026-07-29

- **验收筛选**：待验收可按项目 / 来源（正式待验·变更会话）筛选
- **打回=提 Bug**：打回可写补充、严重度，默认写入项目 Bug；工作台新增「未关 Bug」列表

## v1.10.44 · 2026-07-29

- **待验收收紧**：只保留正式「待验收」+ 未收口变更会话；已标完成 / 已上版条目免验（不再灌几百条）
- **工作台顺序**：星球/日历 → 项目库 → 今日要做与现场并列

## v1.10.43 · 2026-07-29

- **MCP 版本独立**：`lib/mcp/version.ts` 的 `MCP_SERVER_VERSION`（现 1.4.0）只随 MCP 工具/鉴权变更升号，不再跟页面 `package.json`

## v1.10.42 · 2026-07-29

- **版本对齐（已由 1.10.43 纠正）**：曾误把 MCP version 绑到 package.json；现改为 MCP 独立版本

## v1.10.41 · 2026-07-29

- **工作台=PM今日桌面**：「今日要做」含宽口径待验收（正式待验 / 近14天已标完成未通过 / 变更会话 / 上版）+ 需你跟进 + 明日清单（仅到期=明天）；行内通过/打回拧状态
- **信息架构**：砍「最近完成」与「进行中需求」主栏；灵感/演进/Git 收入可折叠「现场」；`/todos` 重定向到工作台今日区

## v1.10.40 · 2026-07-29

- **改进日历**：日总结的「完成」改为「上版」（按演进 `releaseTag`），不再把补标旧需求的 completed_at 当成当天完成
- **MCP**：`list_evolutions` / `update_evolution` / `repair_corrupt_evolution_titles`（按 CHANGELOG 修复标题问号乱码）

## v1.10.39 · 2026-07-29

- **工作台布局**：项目库提到需求区前面；项目库 / 进行中需求均可折叠（偏好存本机）；需求区默认收起，方便一眼扫全项目
- **改进日历**：选日展示每日总结（改了 x 项目、完成 x 需求、主要方向、分项目方向与条目），不再只罗列项目名

## v1.10.38 · 2026-07-29

- **明日待办**：只收明天真要做的（昨日变更未完 / 会话未勾完 / 到期=明天）；不再塞入全部 Studio 未完成任务；完整待办仍在「我的待办」其它区
- **概况工时**：消耗汇总演进/变更会话聊天工时
- **桥接**：随心而行 PM slug 用 `studio-proj-moonpie`（勿硬编码 `moonpie`）

## v1.10.37 · 2026-07-29

- **MCP 真实私域**：Bearer / OAuth 通过后进入管理员作用域，不再误进「晨光手记」演示沙盘
- **MCP**：`list_requirements` / `update_requirement`（可标完成、写 completed_at）
- **概况需求进度**：每条固定显示 P0–P3；状态跟 lifecycle 标签走
- **桥接**：`proj-moonpie` → PM slug `moonpie`

## v1.10.36 · 2026-07-28

- **审慎协作**：不假设用户已想清楚；从原始需求出发；动机/目标不清则停下讨论；逻辑漏洞必须提醒；方案须全链路逻辑验证后再做/再发版

## v1.10.35 · 2026-07-28

- **协作节奏**：要求不清先细问再做；计划/需求直接写 PM，不写 spec；发版前测试对照**初始要求**
- **Skill**：做完可发版；有则写之无则跳过；发版前测试门禁  
- 发版顺序修正：先 `git tag`+push，再 `publish_release`（避免远程抢建错位 tag）
...
- **板块进程表**：迭代记录「板块演进」改为表视图（板块 / 提出时间 / 改进时间线 / 工时）；大→子缩进；提出时间=灵感与演进取更早
- **聊天工时**：演进新增 `work_started_at` / `work_finished_at`；变更会话用 created/finished；MCP `add_evolution` / `start|finish_change_session` 可传时间戳；条目 + 板块行 Σ
- **星球日历**：工作台星图旁可切换日历，按日看演进 + AI 变更会话（项目 / 板块）
- **需求状态机**：看板 `想法 → 已规划 → AI开发中 → 待验收 → 完成`（旁路放弃）；旧标签归一
- **AI 写入/发版 Skill**：仓库 `.cursor/skills/star-pm-write-release`；字段「有则写之，无则跳过」；alwaysApply 规则发版/写入前必读

## v1.10.30 · 2026-07-28

- **需求状态机**：看板固定列改为 `想法 → 已规划 → AI开发中 → 待验收 → 完成`（旁路 `放弃`）；旧标签（待开始/评审/开发中…）自动归一；表格与详情可下拉改状态；入迭代时「想法」升为「已规划」；测试通过→「待验收」
- **随心而行时间线**：种子/ mock 起点对齐 git Initial commit **2026-07-06**（非 3 月）

## v1.10.29 · 2026-07-28

- **公开演示**：生产开启鉴权后，未登录也可逛「晨光手记」沙盘；右上角登录管理员看全量；密钥/写操作仍要管理员。未登录 ≠ 裸奔全库。

## v1.10.28 · 2026-07-28

- **观看者演示沙盘**：`VIEWER_*` 登录后只见虚构项目「晨光手记（演示）」（`proj-demo-showcase`），含精选灵感/任务/演进/需求池；侧栏标「观看者 · 演示数据」；写操作为只读

## v1.10.27 · 2026-07-28

- **从 Git 检测完成**：匹配改为「长中文短语 / 英文标识命中 1 个即可」；同时扫描已同步的 Release/Tag body（不再要求凑满 2 个关键词）

## v1.10.26 · 2026-07-28

- **粘贴脑暴拆任务**：确认入库时同步写入需求池表格行（父 epic + 勾选子功能），并链到灵感；默认仍写 Studio 任务

## v1.10.25 · 2026-07-27

- **明日待办清单**：今日工作台 / 我的待办自动生成；优先「前一天有变更且未完成」的需求 + 变更会话未勾完项 + Studio 未完成任务；按优先级排序；可筛项目

## v1.10.24 · 2026-07-27

- **灵感流**：新增「粘贴脑暴 → 自动拆任务」：选项目、粘贴全文、预览勾选、确认后写 1 条灵感 + 多条 Studio 任务
- 有 OpenAI Key 时优先 AI 拆；失败或未配置则按 ①② / 标题 / 序号规则切分

## v1.10.23 · 2026-07-27

- **AI 变更会话 ChangeSession**：改前目标/原因/期望，改后执行勾选与 AI 操作摘要，人工验收（未验收/通过/退回）
- **MCP**：`start_change_session` / `finish_change_session` / `update_change_session` / `list_change_sessions` / `get_change_session`
- **迭代记录**：新增 Tab「AI 变更」，按日汇总 + 下钻编辑
- **Migration**：`032_studio_change_sessions.sql`
- **项目库**：表格内嵌双向滚动；板块演进改为板块/子板块下拉筛选

## v1.10.22 · 2026-07-23

- **Excel**：支持「一级/二级/三级模块」列导入与导出；绑定仍为根 + 叶子（`module_l2_id` 挂最深节点）
- **需求池**：筛选增加「根模块」
- **甘特**：可按模块筛选（含任意层级与未分组）

## v1.10.21 · 2026-07-23

- **模块树任意多层**：取消两级上限；任意节点可加子模块
- **功能板块粘贴**：支持 `·` `/` `、` 自动分层（如 `六爻/笔记`、`六爻、笔记、卦象解析`）；同名同父合并
- **默认展示两层**：更深层级默认收起，可点开展开
- 迁移：`031_modules_arbitrary_depth.sql`（去掉 `level in (1,2)` 约束）

## v1.10.20 · 2026-07-23

- **功能板块 → 模块树**：保存 / MCP 写入 `featureModules` 时，按首个「·」拆成一级 + 子模块并增量导入（同名跳过）
- 概览页模块区增加「从功能板块导入」按钮

## v1.10.19 · 2026-07-22

- **MCP `update_project`**：支持写入 `featureModules`（功能板块名单，完整覆盖）；`slimProject` 回传该字段

## v1.10.18 · 2026-07-21

- **需求池**：多选「迁移」对话框——先选目标项目、再选目标计划（需求池 / 一期二期等）；选中父节点时整棵子树一并迁走

## v1.10.17 · 2026-07-21

- **需求池**：修正固定列宽不一致导致「类型」等后续列被遮挡显示不全

## v1.10.16 · 2026-07-21

- **通用列宽**：抽出 `useColWidths` / `ResizableTh`（灵感流表格已接入）
- **跨设备偏好**：`studio_app_settings` + `/api/studio/app-settings`；工作台项目库、需求总览视图、项目库列宽/顺序、灵感流列宽、需求池列宽写入后同步；进入工作台自动 hydrate
- **迁移**：需有 `030_studio_app_settings.sql`（线上 MCP 已建表）

## v1.10.15 · 2026-07-21

- **灵感流表格**：可拖改列宽；表内筛选（搜索 / 优先级 / 类型 / 状态）

## v1.10.14 · 2026-07-21

- **需求池**：表头可拖改列宽（本浏览器记住）；行拖中间挂为子节点、上下半区改顺序；左侧悬停 `+` 建子需求

## v1.10.13 · 2026-07-21

- **需求总览**：Notion 式多条件筛选（项目 / 状态 / 优先级 / 搜索 / 隐藏已完成）+ 可保存/切换/更新/删除视图（本浏览器）

## v1.10.12 · 2026-07-21

- **项目库**：调列宽、左侧 + 建子项目、拖拽排序/挂子节点（一层）、表格↔看板（按状态）
- **需求总览**：看板↔表格切换；项目下拉与「隐藏已完成」两视图共用（Notion 式筛选第二轮）

## v1.10.11 · 2026-07-21

- **工作台 · 阻塞**：点「阻塞 N」展开列表（项目 / 任务 / 阻塞原因），再点进对应项目「Studio 任务」页

## v1.10.10 · 2026-07-21

- **灵感流时间线**：每条直接显示具体日期时间（年-月-日 时:分），不再只显示钟点

## v1.10.9 · 2026-07-21

- **工作台 · 项目库**：内嵌「调整显示」— 开关可见性、拖拽排序；配置存本浏览器；去掉原先只展示前 4 个的限制
