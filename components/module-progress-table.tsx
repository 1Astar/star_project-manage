...
import { ModuleProgressPeek } from "@/components/module-progress-peek";
...
export function ModuleProgressTable({
  ...
  function openNode(kind: "evolution" | "idea" | "change", id: string) {
    if (kind === "evolution") {
      const log = evolution.find((e) => e.id === id);
      if (log) setPeek({ kind: "evolution", log });
      return;
    }
    if (kind === "idea") {
      const idea = ideas.find((i) => i.id === id);
      if (idea) setPeek({ kind: "idea", idea });
      return;
    }
    const session = changeSessions.find((s) => s.id === id);
    if (session) setPeek({ kind: "change", session });
  }
  ...
  return (
    <>
                    ...
                    <ol className="relative max-w-xl space-y-2.5 border-l border-slate-200 pl-3">
                      {row.nodes.map((node) => (
                        <li key={`${node.kind}:${node.id}`} className="relative">
                          ...
                          <button
                            type="button"
                            onClick={() => openNode(node.kind, node.id)}
                            className="w-full rounded-md text-left hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300"
                          >
                            ...
                            <div className="text-[13px] font-medium text-slate-800 hover:text-indigo-700">
                              {node.title}
                            </div>
                            {node.note ? (
      ...
      {peek ? (
        <ModuleProgressPeek target={peek} onClose={() => setPeek(null)} />
      ) : null}
    </>
  );
}
