      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-xs text-stone-400">
            {doneCount}/{tasks.length} 已完成 · 点标题或「详情」展开备注与 Git
          </p>
        </div>
        {hasGitHub ? (
          <button
            ...
            {tasks.map((task) => {
              ...
              return (
                <Fragment key={task.id}>
                  <tr className={isOpen ? "bg-indigo-50/50" : "hover:bg-stone-50/80"}>
                    <td className="px-2 py-1.5 align-top">
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <div className="font-medium text-stone-800 underline-offset-2 group-hover:text-indigo-700 group-hover:underline">
                            {task.title}
                          </div>
                          {task.workload ? (
                            <div className="mt-0.5 text-xs text-stone-400">{task.workload}</div>
                          ) : null}
                          {!isOpen && task.progressNote?.trim() ? (
                            <div className="mt-0.5 line-clamp-1 text-[11px] text-stone-400">
                        ...
                        <span
                          className={
                            isOpen
                              ? "shrink-0 rounded border border-indigo-200 bg-indigo-50 px-1.5 py-0.5 text-[11px] text-indigo-700"
                              : "shrink-0 rounded border border-stone-200 bg-white px-1.5 py-0.5 text-[11px] text-indigo-600 group-hover:border-indigo-200"
                          }
                        >
                          {isOpen ? "收起" : "详情"}
                        </span>
                  ...
                  {isOpen ? (
                    <tr className="border-l-2 border-indigo-400 bg-indigo-50/30">
                      <td colSpan={10} className="px-4 py-3">
                        <p className="mb-2 text-xs font-medium text-indigo-800">任务详情</p>
