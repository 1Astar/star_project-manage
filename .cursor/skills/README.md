...
2. 不知道做什么 → 看 Idea **下一步**（当前默认 **P2**，以卡片为准）  
3. 做完 → **勾完成** → 把「下一步」改成**下一级 P**  
4. 文档交付 → 只放成品；格式走 `doc-delivery-formats` 路由  
5. 删文件 → `safe-file-delete`  
6. **改文件前检查用户是否已修改；用户改过则在修改过的基础上再改**（`version-management`）  
7. **文档小修改不新建文件**；**小版本 / 大版本迭代才新增带版本号的文件**（`version-management`）
